...
public class Pr02ArcheryTournament {
    ...
    public static void main(String[] args) throws IOException {
        ...
        reader.lines()
                .takeWhile(command -> !"Game over".equals(command))
                .forEachOrdered(commandProcessor(targets, points));

        System.out.println(targets.stream().map(String::valueOf).collect(Collectors.joining(" - ")));
        System.out.printf("Iskren finished the archery tournament with %d points!", points.get());
    }

    private static Consumer<String> commandProcessor(List<Integer> targets,
        ...
        return command -> {
            if ("Reverse".equals(command)) {
                ...
                int newPoints = shootAtTarget(targets, direction, startIndex, length);
